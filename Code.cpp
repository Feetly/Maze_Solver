...
forward over right.
int timefr; // Dynamically set time..for which it moves forward,when
it chooses forward over right.
int tlbef=440; // Time for which it moves forward before taking left
turn.
...
The below code contains code both for Maze Solving
and Shortest path Simplification.
...
if(leftdist()>=7.2) // Will move little to its left if its too far
from the left wall
{
digitalWrite(10,LOW);
digitalWrite(11,LOW);
...
pinMode(12,OUTPUT);
pinMode(13,OUTPUT);
Serial.begin(9600); //staartingg serial communication...9600 bits per
second.
...
values of five durations... We will take only the median value(afer sorting) with
error bearing capacity of 40%.
float ldista[5];
...
if((fdist>=125)||(rdist>=150)||(ldist>=400)) {return;} // Cancelling out
any error values...goes back to void loop().
...
i = i+1;
j=j+1;
if((found==true)&(dir[i]!=0)) // After reaching the end ... checks the
s
{
rightturn();return;
}
...
/*Serial.print("for the jth turn ..");Serial.print("
=");Serial.print(j);
Serial.print(" the i value is ");Serial.print(i);Serial.print("and the
dir is ..");Serial.println(dir[i]);*/
timefr = tfr + 65*nf;
nf=nf+1;
stopit();delay(1000);
